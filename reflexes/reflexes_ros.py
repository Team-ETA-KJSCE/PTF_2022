...
def broadcastTf(x,y,depth,text):
...
def callback_depth(data):
    ...
    try:
        ...
        for c in coords:
            ...
            depth = frame[c_y][c_x]
            if depth<1:
                broadcastTf(c_x,c_y,depth,text)
            index = index +1
...
def main(args):
    ...
    # subscribing to depth camera
    image_sub_depth = rospy.Subscriber("/prius/front_camera/image_raw1", Image, callback_depth,queue_size=1, buff_size=2**32)

    #image_sub_depth = rospy.Subscriber("/camera/depth/image_raw2", Image, callback_depth)

    #image_pub = rospy.Publisher("debug_image",Image, queue_size=1)
    rate = rospy.Rate(10)
